...
export default function CreateAccountOnlineEmail({ navigation }) {
    ...
    useEffect(() => {
        setPasswordValidityObj(prev => { return { ...prev, length: password.length > 6 } });
        setPasswordValidityObj(prev => { return { ...prev, symbol: !password.match(/^(|[a-zA-Z0-9]*)$/) } });
        setPasswordValidityObj(prev => { return { ...prev, number: password.match(/[0-9]/) } });
        setPasswordValidityObj(prev => { return { ...prev, uppercase: password.match(/[A-Z]/) } });
    }, [password])
                ...
                hasMounted ?
                    <Animated.View
                        ...
                        <RBox figmaImport={{ mobile: { left: 50, width: 120, height: 60, top: 283 } }}>
                            ...
                            <RLabel color={passwordValidityObj.number ? globalStyle.successTextColor : globalStyle.textColor} left="11%" width='90%' align='left' alignPadding='0%' text='At least a number' top={30} fontSize={10}></RLabel>
                            <RLabel color={passwordValidityObj.uppercase ? globalStyle.successTextColor : globalStyle.textColor} left="11%" width='90%' align='left' alignPadding='0%' text='An uppercase letter' top={45} fontSize={10}></RLabel>
                            <RBox borderRadius={2} height={10} width={10} top={3.5 * 1} style={{ left: '0%' }} backgroundColor={passwordValidityObj.length ? globalStyle.successColor : globalStyle.textColor}></RBox>
                            <RBox borderRadius={2} height={10} width={10} top={3.5 * 5.5} style={{ left: '0%' }} backgroundColor={passwordValidityObj.symbol ? globalStyle.successColor : globalStyle.textColor}></RBox>
                            <RBox borderRadius={2} height={10} width={10} top={3.5 * 10} style={{ left: '0%' }} backgroundColor={passwordValidityObj.number ? globalStyle.successColor : globalStyle.textColor}></RBox>
                            <RBox borderRadius={2} height={10} width={10} top={3.5 * 14} style={{ left: '0%' }} backgroundColor={passwordValidityObj.uppercase ? globalStyle.successColor : globalStyle.textColor}></RBox>
                        </RBox>
                        <RButton onClick={() => { navigation.navigate('createAccountOnline', { name: 'createAccountOnline' }) }} isEnabled={true} color={globalStyle.textColorAccent} borderColor={globalStyle.colorAccent} backgroundColor={globalStyle.colorAccent} figmaImport={{ mobile: { top: 449, left: 50, width: 260, height: 38 } }} alignPadding={'3%'} verticalAlign='center' fontSize={14}>
