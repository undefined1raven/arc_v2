...
function globalEnteringConfig(duration?: number, delay?: number, transitionIndex?: number) {
    return FadeInDown.delay(getVal(delay, 40) * getVal(transitionIndex, 0)).duration(getVal(duration, 50)).easing(Easing.inOut(Easing.ease)).withInitialValues({ transform: [{ translateY: -50 }] })
}
